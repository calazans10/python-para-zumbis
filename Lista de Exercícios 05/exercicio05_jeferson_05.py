...
def has_equal_two_digits(number):
...
def has_equal_first_and_last_digits(number):
    number = str(number)
...
for telefone in telefones:
    if is_even_sum_of_digits(telefone) and not \
            has_equal_first_and_last_digits(telefone) and not \
            has_equal_two_digits(telefone):
        validos += 1
